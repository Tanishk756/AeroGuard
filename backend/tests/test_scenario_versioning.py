"""Stage S6 Scenario Versioning & Duplication Test Suite."""
...
def sample_scenario(database):
    ...
    scen = PersistentScenarioEntity(
        id="scen-vers-orig",
        project_id="proj-default-01",
        name="Original Scenario v1",
        vehicle_id=vehicle.id,
        simulator="GAZEBO",
        autopilot="ARDUPILOT",
        world_id=world.id,
        environment_config_json={"gravity": [0, 0, -9.81]},
        physics_config_json={"step_size_s": 0.004},
        weather_config_json={"wind_speed_m_s": 2.0},
        spawn_config_json={"position": [0, 0, 0.2]},
        random_seed=42,
...
def test_scenario_versioning_and_duplication(database, sample_scenario):
    """VERIFIED: Duplicating a scenario creates a distinct scenario ID while preserving specs."""
    orig = sample_scenario
    dup = PersistentScenarioEntity(
        id="scen-vers-dup",
        project_id=orig.project_id,
        name=f"{orig.name} (Copy)",
        vehicle_id=orig.vehicle_id,
        simulator=orig.simulator,
        autopilot=orig.autopilot,
        world_id=orig.world_id,
        environment_config_json=orig.environment_config_json,
        physics_config_json=orig.physics_config_json,
        weather_config_json=orig.weather_config_json,
        spawn_config_json=orig.spawn_config_json,
    ...
    assert dup.name == "Original Scenario v1 (Copy)"
    assert dup.weather_config_json["wind_speed_m_s"] == 2.0
